...
MIN_LEN = 1          # 最小名称长度
MAX_LEN = 8          # 最大名称长度
PREVIEW_LIMIT = 200  # 最大生成数量（0表示不限制）
...
def main():
    ...
    print(f"\n开始回溯剪枝生成名称（长度 {MIN_LEN}~{MAX_LEN}，最多 {PREVIEW_LIMIT} 个）...")
    sys.stdout.flush()
    start_time = time.time()
    names = backtrack_with_pruning(pool, MIN_LEN, MAX_LEN, PREVIEW_LIMIT)
    elapsed = time.time() - start_time

    print(f"生成完成，共 {len(names)} 个名称，耗时 {elapsed:.3f} 秒")
    print("\n前20个示例:")
